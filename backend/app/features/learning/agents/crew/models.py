...
class YoutubeLessonPlanItem(BaseModel):
    """One skill-gap study step (videos + reading). Name kept for API compatibility."""
    ...
    # Optional free-text query for blogs/articles/docs (never invent specific article URLs).
    article_search_query: str | None = Field(default=None, max_length=200)
    ...
    @field_validator("article_search_query")
    @classmethod
    def strip_optional_article_query(cls, value: str | None) -> str | None:
        if value is None:
            return None
        cleaned = value.strip()
